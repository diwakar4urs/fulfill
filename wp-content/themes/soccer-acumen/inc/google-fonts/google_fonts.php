...
if(!function_exists('_action_theme_process_google_fonts')) {
    function _action_theme_process_google_fonts(){
        ...
        $google_fonts_links = soccer_acumen_prepare_google_fonts($include_from_google);
 ...
 * @return 
 */
if (!function_exists('soccer_acumen_prepare_google_fonts')) {
    function soccer_acumen_prepare_google_fonts($include_from_google) {
        $fonts_url = '';
		/**
         * Get remote fonts
        ...
        foreach ( $include_from_google as $font => $font_family ) {
			...
			* into your own language.
			$font_on_off	= '';
			$font_on_off = _x( 'on', $font.' font: on or off', 'soccer-acumen' );
			if ( 'off' !== $font_on_off ) {}
			*/
 ...
 * @return 
 */
if (!function_exists('soccer_acumen_theme_fonts_url')) {
    function soccer_acumen_theme_fonts_url() {
        $fonts_url = '';
		/**
         * Print google fonts link
         */
		// @ These fonts are get from databases, Functionality is done in given above function name as : soccer_acumen_prepare_google_fonts
 ...
 * @return 
 */
if (!function_exists('soccer_acumen_enqueue_google_fonts')) {
	function soccer_acumen_enqueue_google_fonts() {
		
		if ( !is_admin() ) {
			wp_enqueue_style( 'soccer_acumen_theme_google_font', soccer_acumen_theme_fonts_url(), array(), null );
		}
		
		
		//Default google fonts
		$font_families[]	= 'Oswald:300,400,700|Open+Sans:400,700,600';
		$query_args = array(
            'family' => urlencode( implode( '|', $font_families ) ),
            'subset' => urlencode( 'latin,latin-ext' ),
        );
		
		$default_fonts_url = add_query_arg( $query_args, 'https://fonts.googleapis.com/css' );
		wp_enqueue_style( 'soccer_default_google_font', $default_fonts_url, array(), null );
		
		
	}
	add_action( 'wp_enqueue_scripts', 'soccer_acumen_enqueue_google_fonts' );
}