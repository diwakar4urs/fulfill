...
if (isset($atts['parallax']) && $atts['parallax'] === 'on' && isset($atts['background_image']['data']['icon']) && !empty($atts['background_image']['data']['icon'])) {
    ...
    $paralaxx_configuration = 'data-speed="' . $parallax_speed . '" data-bleed="' . $parallax_bleed . '" data-appear-top-offset= "600" data-parallax="scroll" data-position="center 0" data-image-src="' . $atts['background_image']['data']['icon'] . '"';
...
<?php if (isset($sidebar_status) && $sidebar_status === 'on') { ?>
    <section  class="tg-main-section haslayout tg-twocolumns section-with-sidebar <?php echo sanitize_html_class($section_type) ?> <?php echo implode(' ', $section_extra_classes) ?>" <?php echo ($custom_id); ?> <?php echo ($section_style); ?> <?php echo ($bg_video_data_attr); ?> <?php echo ($paralaxx_configuration); ?>>
        
        <div class="builder-two-column">
            <?php if (!empty($atts['section_heading'])) { ?>
                <div class="tg-section-name"><h2><?php echo esc_attr($atts['section_heading']); ?></h2></div>
            <?php } ?>
            <div class="col-sm-8 col-xs-12 <?php echo sanitize_html_class($section_position); ?>">
                <div class="row">
                        ...
                        <div class="builder-items">
                            <?php if (isset($atts['section_heading']) && !empty($atts['section_heading'])) { ?>
                                <h2 class="tg-section-title"><?php echo esc_attr($atts['section_heading']); ?></h2>
                                 <?php 
									$wraper_start	= '<div class="col-sm-11 col-xs-11 pull-right"><div class="row">';
									$wraper_end	= '</div></div>';
								 ?>
								<?php } ?>
                            <?php 
            ...
            <div class="col-sm-4 col-xs-12 <?php echo sanitize_html_class($sidebar_position); ?>">
