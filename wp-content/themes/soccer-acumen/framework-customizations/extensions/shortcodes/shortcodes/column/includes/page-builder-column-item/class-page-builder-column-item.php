...
class Page_Builder_Column_Item extends Page_Builder_Item {
	...
	private function get_item_data() {
		...
		$data['l10n'] = array(
			'edit' => esc_html__( 'Edit', 'soccer-acumen' ),
			'duplicate' => esc_html__( 'Duplicate', 'soccer-acumen' ),
			'remove' => esc_html__( 'Remove', 'soccer-acumen' ),
			'title' => esc_html__( 'Column', 'soccer-acumen' ),
		);
	...
	protected function get_thumbnails_data() {
		...
		foreach ( $builder_widths as $key => $value ) {
			$column_thumbnails[ $key ] = array(
				'tab'         => esc_html__( 'Layout Elements', 'soccer-acumen' ),
				...
				'description' => apply_filters( 'fw_ext_shortcodes_column_description',
					sprintf( esc_html__( 'Add a %s column', 'soccer-acumen' ), $value['title'] ), $key ),
