...
if (!class_exists('TGFlickr')) {
    ...
    class TGFlickr extends WP_Widget {
        ...
        public function __construct() {
            $widget_ops = array('classname' => 'flicker-column', 'description' => 'To Displays Flickr Gallery.');
            $control_ops = array('width' => 300, 'height' => 250, 'id_base' => 'tg_flickr');
            parent::__construct('tg_flickr', esc_html__('TG Flickr Widget', 'soccer-acumen'), $widget_ops, $control_ops);
        }
        ...
        public function form($instance) {
            if (isset($instance['title']) && isset($instance['username'])) {
                ...
                $title  = esc_html__('Flickr Photos', 'soccer-acumen');
                $username = esc_html__('', 'soccer-acumen');
				$no_of_photos 	= 8;
            }
            ?>
            ...
            <p>
                <label for="title">
                    <?php esc_html_e('Title', 'soccer-acumen'); ?>
                </label>
            ...
            <p>
                <label for="username">
                    <?php esc_html_e('User Name', 'soccer-acumen'); ?>
                </label>
			...
			<p>
                <label for="no_of_photos">
                    <?php esc_html_e('No of Photos', 'soccer-acumen'); ?>
                </label>
        ...
        public function widget($args, $instance) {
			...
			if ($apiKey <> '') {
                ...
                if ($check_transient === false || ($apiKey <> $db_apiKey || $username <> $db_user_name || $no_of_photos <> $db_total_photos)) {
                    ...
                    if ($user_info['stat'] == 'ok') {
                        ...
                        if (is_wp_error($response)) {
                            ...
                            foreach ((array) $content['photos']['photo'] as $single_photo) {
                                ...
                                if (isset($image_file) && !empty($image_file) ) {
                                    ...
                                    $return	.= '<a target="_blank" class="flaticon-plus79" title="'.$single_photo['title'] .'" href="https://www.flickr.com/photos/'.$single_photo['owner']."/".$single_photo['id'].'">
													<img alt="'.$single_photo['title'].'" src="'.esc_url( $image_file ) . '">
												</a><span class="img-overlay"><a href="'.esc_url( $image_file ) . '">+</a></span>';
								    $return	.=  '</li>';
                                    $counter++;
                            ...
                            if ($counter == 0)
                                esc_html_e('No result found.' , 'soccer-acumen');
                        }
                    } else {
                        echo esc_html__('Error:' , 'soccer-acumen') . $user_info['code'] . ' - ' . $user_info['message'];
                    }
                } else {
                        ...
                        if (isset($flick_data['photo_src'])):
                            ...
                            foreach ($flick_data['photo_src'] as $ph) {
								...
								$return	.= '<a target="_blank" class="flaticon-plus79" title="'.$flick_data['photo_title'][$i] .'" href="https://www.flickr.com/photos/'.$flick_data['photo_owner'][$i]."/".$flick_data['photo_id'][$i].'">
												<img alt="'.$flick_data['photo_title'][$i].'" src="'.esc_url( $flick_data['photo_src'][$i]) . '">
											</a><span class="img-overlay"><a href="'.esc_url( $flick_data['photo_src'][$i]) . '"">+</a></span>';
								$return	.=  '</li>';
                                $i++;
                            }
                        endif;
                ...
                esc_html_e('Please set API key first. Please go to Plugin Options' , 'soccer-acumen');
            }
